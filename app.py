...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://postgres:Anhchuc123@localhost/NTHERESAHW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
class User(UserMixin, db.Model):
    ...
    gif_collections = db.relationship('PersonalGifCollection', backref='User')
...
def get_or_create_search_term(term):
    ...
    if search:
        ...
        gif_data = get_gifs_from_giphy(term)
        for gif in gif_data:
            g = get_or_create_gif(title= gif['title'], url = gif['embed_url'])
            search.gifs.append(g)
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    if collection:
        ...
        collection = PersonalGifCollection(name=name, user_id=current_user.id, gifs=gif_list)
        for gif in gif_list:
...
def index():
    ...
    if form.validate_on_submit():
        ...
        search_term = get_or_create_search_term(search)
...
def create_collection():
    ...
    if request.method == 'POST'and form.validate_on_submit():
        ...
        gif_list = []
        for id in selected_gifs:
            j = get_gif_by_id(int(id))
            gif_list.append(j)
        get_or_create_collection(name=name, current_user=current_user, gif_list=gif_list)
